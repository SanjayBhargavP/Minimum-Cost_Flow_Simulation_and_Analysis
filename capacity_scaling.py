...
def capacity_scaling_with_metrics(graph, source, sink, demand):
    ...
    while scaling_factor >= 1:
        while demand > 0:
            path, bottleneck = GraphHelper.bellman_ford_capacity_scaling(graph, source, sink, scaling_factor)
            if not path:
                break
    ...
    if demand > 0:
        ...
        longest_acyclic_path = GraphHelper.find_longest_acyclic_path(graph, source, sink)
